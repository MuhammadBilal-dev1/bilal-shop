...
import fetcgCategoryWiseProduct from "../helpers/fetchCategoryWiseProduct";
...
const VerticalCardProduct = ({ category, heading }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    const categoryProduct = await fetcgCategoryWiseProduct(category);
    setLoading(false);
  ...
  return (
    <div className="container mx-auto px-4 my-6 relative">
      ...
      >
        <button
          className="bg-white shadow-md rounded-full p-1 h-20 hover:bg-blue-200 hover:text-white absolute left-0 text-lg hidden md:block"
          onClick={scrollLeft}
        >
          <FaAngleLeft />
        </button>
        <button
          className="bg-white shadow-md rounded-full p-1 h-20 hover:bg-blue-200 hover:text-white absolute right-0 text-lg hidden md:block"
          onClick={scrollRight}
        >
          <FaAngleRight />
        </button>
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4 grid gap-3">
                    ...
                    <button className="text-sm text-white px-3 rounded-full p-1 animate-pulse  bg-slate-200 py-2"></button>
                  </div>
                </div>
              );
            })
